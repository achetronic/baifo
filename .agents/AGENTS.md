...
│   │   ├── allproviders/           # blank-imports the three below
│   │   ├── openai/                 # also serves any OpenAI-compatible endpoint
│   │   ├── anthropic/
│   │   └── gemini/
...
LLM providers. Today the registry supports `anthropic`, `openai` and `gemini`.
Any OpenAI-compatible endpoint (a local Ollama server, OpenRouter, vLLM, ...) is
an `openai` provider with a custom `url`; the openai package serves all of them.
...
reasoning effort to the task. A provider whose type has no fixed catalogue,
or an `openai` provider pointed at a custom `url`, returns a note rather than a
catalogue. The lookup is offline; no provider `/models` API call.
...
**Custom-endpoint caveat.** An `openai` provider pointed at a custom `url`
shares the openai adapter, so a `reasoning` value *is* sent as `reasoning_effort`.
Whether it works depends on the endpoint: some accept it, some ignore it, some
error on an unknown field.
...
an explanation; an unknown model (a custom-endpoint id catwalk does not
catalogue, or nothing typed
