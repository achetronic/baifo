...
runtime:
  ...
  retry:
    ...
    backoff:
      ...
      multiplier: 2.0        # each wait is the previous one times this (1s, 2s, 4s ...)
...
secrets:
  ...
  # would otherwise rewrite unrelated text. A higher value means fewer
  # false positives but short secrets bypass the scan; a lower value
  # means more coverage but more risk. 8 covers every real API key; 0
  # disables the floor (not recommended).
  min_scrub_length: 8
...
#                                  # Any OpenAI-compatible endpoint (Ollama,
#                                  # OpenRouter, vLLM, ...) is type: openai + url
...
mcps:
  - name: filesystem
    ...
    # file read or search can return, so one accidental 'cat' of a huge
    # file cannot flood the model's memory. The model is told about these
...
# the TUI and over A2A. The root below is ready to go except for its
# model. Set llm.provider and llm.model (via the /agent command or by
# hand) once you have added a provider to baifo.yaml. Until then baifo
...
#   /secret delete NAME
#   /secret list
#   /secret encode    (encrypt every plaintext entry on disk)
#   /secret decode    (decrypt every entry into plaintext)
#
