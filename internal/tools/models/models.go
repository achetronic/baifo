...
// no network call to the provider's /models endpoint. Provider types
// with no fixed catalogue (openai-compatible, ollama) report a note
// instead of an empty list so the LLM does not assume the provider has
// no models at all.
package models
...
// spec) and its Type (gemini, openai, anthropic, ...), which is what we
// map to the catwalk catalogue.
...
func (t *Tools) listModels(a listModelsArgs) ListModelsResult {
	...
	for _, ref := range refs {
		...
		out = append(out, buildProviderModels(ref, catalogue))
	}
...
// catwalk index. Unknown types (custom endpoints) get a note so the
// LLM understands the catalogue can't enumerate them, not that they
// are empty.
func buildProviderModels(ref ProviderRef, catalogue map[string]catwalk.Provider) ProviderModels {
	pm := ProviderModels{Provider: ref.Name, Type: ref.Type}

	cw, ok := catalogue[strings.ToLower(ref.Type)]
	if !ok {
		pm.Note = "no built-in model catalogue for provider type " + quote(ref.Type) +
			" (e.g. openai-compatible or ollama). Use any model id the endpoint serves; " +
...
// indexCatwalk builds a type->provider map from catwalk's embedded
// catalogue, keyed by the lowercase provider ID so the baifo provider
// type (also lowercase: "gemini", "openai", "anthropic") matches.
func indexCatwalk() map[string]catwalk.Provider {
	all := embedded.GetAll()
	idx := make(map[string]catwalk.Provider, len(all))
	for _, p := range all {
		idx[strings.ToLower(string(p.ID))] = p
	}
	return idx
}
